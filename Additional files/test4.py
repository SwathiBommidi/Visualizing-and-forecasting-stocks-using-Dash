...
input_length = 360
feature_count = 1
generator = TimeseriesGenerator(train, train, length=input_length, batch_size=120)
...
add_dates = [data_frame.index[-2] + DateOffset(days=x) for x in range(0, 361)]
...
df_projection['UpperLimit'] = df_projection['UpperLimit'] + 100
df_projection['LowerLimit'] = df_projection['LowerLimit'] - 100
